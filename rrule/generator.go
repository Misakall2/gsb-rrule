...
// generateWalls generates ordered wall-clock start times up to and
// including until. When anchor is true (a main RRULE), DTSTART is
// always the first instance per RFC 5545 even when the BYxxx parts
// would not generate it. EXRULE expansion passes false: it produces
// only instances the rule itself generates, which makes its own COUNT
// count the removed instances rather than DTSTART.
